...
- API:
    - ```GET /players/{game_name}/{tag_line}``` — Загружает профиль игрока из API Riot, сохраняет/обновляет локальную базу данных и возвращает данные об игроке.
    ...
    - ```POST /admin/players/{puuid}/update``` — Обновляет рейтинг игрока и историю матчей для существующего игрока.
